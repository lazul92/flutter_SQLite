...
import 'package:sql_litle_crud/contact_lsit.dart';
...
class _MyHomePageState extends State<MyHomePage> {
          ...
          child: new Column(
            children: <Widget>[
              TextFormField(
                ...
                validator: (val) => val.length == 0 ? 'Enter onnce Name' : null,
                onSaved: (val) => this.name = val,
              ),
              TextFormField(
                keyboardType: TextInputType.text,
                decoration: new InputDecoration(labelText: 'Phone'),
                validator: (val) => val.length == 0 ? 'Enter onnce Phone number' : null,
  ...
  void startContactList() {
    Navigator.push(context, new MaterialPageRoute(
        builder: (context) => new MyContactList()
      )
    );
  }
}
